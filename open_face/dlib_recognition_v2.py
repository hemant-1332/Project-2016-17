...
client.connect(("172.17.27.180",12345))
...
def sending_information(track_locs):
    # assert lenq(track_locs)==len(face_names), 'Error in extraction'
    # sending_info = []
    # for elem in xrange(len(track_locs)):
    #     sending_info.append(str(track_locs[elem]) + str(face_names[elem]))
    # sending_info = track_locs + str(face_names)
    si = ''.join(track_locs)
    client.send(si)
    print si
